...
class Feature_Pool(nn.Module):
    ...
    def forward(self, x):
        b, c, _, _ = x.size()
        y = self.up(self.act(self.down(self.gap_pool(x).permute(0, 2, 3, 1)))).permute(
            0, 3, 1, 2).view(b, c)
        return y
...
class Channel_Attention(nn.Module):
    ...
    def forward(self, x):
        max_out = self.up(self.act(self.down(self.gap_pool(
...
class EAEF(nn.Module):
    def __init__(self, dim):
        ...
        self.dwconv = nn.Conv2d(
            dim*2, dim*2, kernel_size=7, padding=3, groups=dim)
    ...
    def forward(self, RGB, T):
        ...
        cross_gate = torch.diagonal(
            torch.sigmoid(logits_per)).reshape(b, c, 1, 1)
...
class aggregation_init(nn.Module):
    def __init__(self, channel, mode='None'):
        ...
        if mode == 'out':
            self.conv = nn.Conv2d(3*channel, 11, 1)
        if mode == 'splat':
...
class FA_encoder(nn.Module):
    ...
    def forward(self, input):
        ...
        if verbose:
            print("rgb.size() after layer2: ", rgb_2_1.size())  # (60, 80)
        thermal_2 = self.encoder_thermal_layer2(thermal_1)
        if verbose:
            print("thermal.size() after layer2: ",
                  thermal_2.size())  # (60, 80)
        ...
        if verbose:
            print("rgb.size() after layer3: ", rgb_3_1.size())  # (30, 40)
        thermal_3_1 = self.encoder_thermal_layer3(thermal_2)
        if verbose:
            print("thermal.size() after layer3: ",
                  thermal_3_1.size())  # (30, 40)
        ...
        if verbose:
            print("rgb.size() after layer4: ", rgb_4_1.size())  # (15, 20)
        thermal_4_1 = self.encoder_thermal_layer4(thermal_3_1)
        if verbose:
            print("thermal.size() after layer4: ",
                  thermal_4_1.size())  # (15, 20)
...
class Turbo_decoder(nn.Module):
    def __init__(self, n_class=11, channel=64):
